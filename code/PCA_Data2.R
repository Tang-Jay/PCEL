...
SARAR_Data <- function(m,error=1,nsim=1000){
  ...
  for(p in ps){
    ...
    cat(n,p,f0/nsim,f1/nsim,f2/nsim,f3/nsim,f4/nsim,'\n')
    
    # 储存数据
    CP[j,]  =  c(p,f0/nsim,f1/nsim,f2/nsim,f3/nsim,f4/nsim)
  }
  # 写入到本地
  write.csv(PCEL1, file = paste0('PCEL2-',error,'-',n,'.csv'), row.names = FALSE)
