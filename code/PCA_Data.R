...
library('terra')
library('spdep')
SARAR_Data <- function(m,error=1,nsim=1000){
  # ==================修改参数====================
  c = 3
  rou1 = 0.85 
  rou2 = 0.15
  ...
  PCEL1 = matrix(NA,nsim,length(ps))
  colnames(PCEL1)=ps
  
  CP = matrix(NA,length(ps),6)
  colnames(CP)=c('p','EL','PCA1','PCA2','PCA3','PCA4')
  
  cat('nsim =',nsim,'error =',error,'\n')
  cat('n  ','p ',' EL ',' PCAEL1 ',' PCAEL2 ',' PCAEL3 ',' PCAEL4 ',' MEL','\n')
  j = 0
  for(p in ps){
    j  = j + 1
    ...
    for(i in 1:nsim){
      ...
      # 计算EL值
      lam0 = lambdaChen(z)
      el = 2*sum(log(1+t(lam0)%*%t(z)))
      if(el<=qchisq(0.95,p+3)) f0 = f0+1
      
      # 计算MEL值
      mel = (el-p-3)/sqrt(2*(p+3))
      if(abs(mel)<=qnorm(0.975)) f5 = f5 + 1
      ...
      # E1 = E[,1:s]
      # pcaz1=z%*%E1
      # lam1=lambdaChen(pcaz1)
      # pcel1=2*sum(log(1+t(lam1)%*%t(pcaz1)))
      # if(pcel1<qchisq(0.95,s)) f1 = f1 + 1
      
      # # 计算PCAEL-2值
      # s = 2
      # E2 = E[,1:s]
      # pcaz2=z%*%E2
      # lam2=lambdaChen(pcaz2)
      # pcel2=2*sum(log(1+t(lam2)%*%t(pcaz2)))
      # if(pcel2<qchisq(0.95,s)) f2 = f2 + 1
      
      # # 计算PCAEL-3值
      # s = 3
      # E3 = E[,1:s]
      # pcaz3=z%*%E3
      # lam3=lambdaChen(pcaz3)
      # pcel3=2*sum(log(1+t(lam3)%*%t(pcaz3)))
      # if(pcel3<qchisq(0.95,s)) f3 = f3 + 1
      
      # # 计算PCAEL-4值
      # s = 4
      # E4 = E[,1:s]
      # pcaz=z%*%E4
      # lam4=lambdaChen(pcaz)
      # pcel=2*sum(log(1+t(lam4)%*%t(pcaz)))
      # if(pcel<qchisq(0.95,s)) f4 = f4 + 1
    ...
    cat(n,p,f0/nsim,f1/nsim,f2/nsim,f3/nsim,f4/nsim,f5/nsim,'\n')
    
    # 储存数据
    # CP[j,]  =  c(p,f0/nsim,f1/nsim,f2/nsim,f3/nsim,f4/nsim)
  }
  # 写入到本地
  # write.csv( EL, file = paste0('EL-',error,'-',n,'.csv'), row.names = FALSE)
  # write.csv(MEL, file = paste0('MEL-',error,'-',n,'.csv'), row.names = FALSE)
  # write.csv(PCEL1, file = paste0('PCEL1-',error,'-',n,'.csv'), row.names = FALSE)
  # write.csv( CP, file = paste0('CP-' ,error,'-',n,'.csv'), row.names = FALSE)
}
#===============================================
#                   赋值运算            
#=============================================== 
